...
def test_run_decoded_never_raises_on_bad_bytes(monkeypatch):
    bad = b"SERVICE_NAME: postgresql-x64-16\x98\x00OK"
    monkeypatch.setattr(
        db_bootstrap.subprocess, "run", lambda *a, **k: SimpleNamespace(stdout=bad)
    )
    output = db_bootstrap._run_decoded(["sc", "query"], timeout=5)
    assert "SERVICE_NAME" in output


def test_find_windows_pg_service_parses_name(monkeypatch):
    monkeypatch.setattr(
        db_bootstrap, "_run_decoded", lambda args, timeout: "SERVICE_NAME: postgresql-x64-16\nSTATE: STOPPED"
    )
...
def test_find_windows_pg_service_returns_none_when_absent(monkeypatch):
    monkeypatch.setattr(db_bootstrap, "_run_decoded", lambda args, timeout: "SERVICE_NAME: SomeOtherService")
...
@pytest.mark.asyncio
async def test_ensure_postgres_ready_service_wont_start_reports_service_name(monkeypatch):
    monkeypatch.setattr(db_bootstrap.asyncpg, "connect", AsyncMock(side_effect=ConnectionRefusedError()))
    monkeypatch.setattr(db_bootstrap, "_is_local", lambda host: True)
    monkeypatch.setattr(db_bootstrap.sys, "platform", "win32")
    monkeypatch.setattr(db_bootstrap, "_find_windows_pg_service", lambda: "postgresql-x64-16")
